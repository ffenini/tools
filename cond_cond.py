...
colors=[]
marklist=['o','v','^','<','>','1','2','<','>','1','2','3','4','8','s','p','P','*','h','H','+','x','X','D',
...
for i in zarina:
   ...
   # print(np.where(1/condata[i]['micro_forward (Ohm)'].values==max(1/condata[i]['micro_forward (Ohm)'].values)))
    #print(np.where(log10(1/(u_mean*(cross_sec/v_probe)))==min(log10(1/(u_mean*(cross_sec/v_probe))))))
    maximus=np.where(1/u_mean==np.nanmax(1/u_mean))[0]

    minimus=np.where(1/u_mean==np.nanmin(1/u_mean))[0]
    if len(maximus)>=1:
        maxim=maximus[0]
    else:
        maxim=maximus
    ...
    #print(i,minim)
    if i in [22,24]:
        ax.plot(1000/(condata[i]['T sample a (o^C)'].values[minim:maxim]+273.16),
                    log10(1/(u_mean[minim:maxim]*(cross_sec/v_probe))),
                    marker=mark,markersize=size,mew=mew_size,
                markevery=1,fillstyle='none',label=labby,color=colors[zarina.index(i)])
    else:
        ax.plot(1000/(condata[i]['T sample a (o^C)'].values[maxim:minim]+273.16),
                    log10(1/(u_mean[maxim:minim]*(cross_sec/v_probe))),
                    marker=mark,markersize=size,mew=mew_size,
                markevery=1,fillstyle='none',label=labby,color=colors[zarina.index(i)])
    #else:
    ...
    ax4.set_xticklabels(tticks,fontsize=15,rotation='90')
