...
ax3=plt.subplot(gs[0],sharex=ax5)
ax4=plt.subplot(gs[1],sharex=ax5)
...
x0,x1,x2,x3,x4,x5=test0['2theta']-1.14,test1['2theta']-1.15,test2['2theta']-1.14,test3['2theta'],test4['2theta'],test5['2theta']
...
for n in nitro_ref:
    ...
    for trf in range(len(xN)): #only takes a value per x value, NO DUPLICATES
        if trf==0:          
            ax4.axvline(xN[trf][0],0,xN[trf][1],linewidth=0.06*hsize_thesis,label=ref_lab[nitro_ref.index(n)],alpha=0.8,color=colors[nitro_ref.index(n)])
        if nitro_ref.index(n)==3 or nitro_ref.index(n)==4: 
            ax4.axvline(xN[trf][0],0,xN[trf][1],alpha=1,linewidth=0.06*hsize_thesis,color=colors[nitro_ref.index(n)],linestyle='--')
            ax3.axvline(xN[trf][0],0,xN[trf][1],alpha=1,linewidth=0.06*hsize_thesis,color=colors[nitro_ref.index(n)],linestyle='--')
            ax5.axvline(xN[trf][0],0,xN[trf][1],alpha=1,linewidth=0.06*hsize_thesis,color=colors[nitro_ref.index(n)],linestyle='--')
        else:
            ax4.axvline(xN[trf][0],0,xN[trf][1],linewidth=0.06*hsize_thesis,alpha=0.8,color=colors[nitro_ref.index(n)])
            ax5.axvline(xN[trf][0],0,xN[trf][1],linewidth=0.06*hsize_thesis,alpha=0.8,color=colors[nitro_ref.index(n)])           
...
#figure labels
ax0.text(0.62,0.75,labby[4],transform=ax0.transAxes,fontsize=hsize_thesis*1.4)
ax1.text(0.62,0.75,labby[0],transform=ax1.transAxes,fontsize=hsize_thesis*1.4)
...
ax3.yaxis.set_label_coords(-0.025, -0.5)
...
fig.savefig('C:/M_drive_docs/Thesis/graphics/chromites_cuNI.pdf')
fig.savefig('C:/M_drive_docs/Thesis/graphics/chromites_cuNI.png')