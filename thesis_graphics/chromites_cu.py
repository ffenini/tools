...
for n in nitro_ref:
    ...
    NITRO=[(nitro['2theta'][x],nitro['Intensity'][x]/max(nitro['Intensity'])) for x in range(len(nitro['2theta']))] #generator
    xN=list(OrderedDict.fromkeys(NITRO))
    for trf in range(len(xN)): #only takes a value per x value, NO DUPLICATES
        if trf==0:          
            ax4.axvline(xN[trf][0],0,xN[trf][1],label=n,alpha=0.7,color=colors[nitro_ref.index(n)])
        else:
            ax4.axvline(xN[trf][0],0,xN[trf][1],alpha=0.7,color=colors[nitro_ref.index(n)])
        ax5.axvline(xN[trf][0],0,xN[trf][1],alpha=0.7,color=colors[nitro_ref.index(n)])
...
ax3.text(0.65,0.75,labby[5],transform=ax3.transAxes,fontsize=hsize_thesis*1.4)  
ax4.text(0.725,0.75,labby[1],transform=ax4.transAxes,fontsize=hsize_thesis*1.4)  
ax5.text(0.725,0.75,labby[3],transform=ax5.transAxes,fontsize=hsize_thesis*1.4)  
...
ax4.legend(bbox_to_anchor=(0.3,2.1),ncol=2,frameon=False,fontsize=hsize_thesis*0.7)
#ax.set_ylim(0,0.4)
...
#fig.savefig('C:/M_drive_docs/Thesis/graphics/chromites_cu.pdf')
#fig.savefig('C:/M_drive_docs/Thesis/graphics/chromites_cu.png')